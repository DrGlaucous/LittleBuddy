 ...
 *
 */
public final class PrimaryViewController//all processes are part of this class
{
	...
	private static final double TRANSITION_DURATION = 50.0;
	...
	private final static String ITEM_STRING_ADVANCED_MODE = "Advanced Mode";
	...
	private void initialize()
	{
		...
		setSvgIcons();
		applyFocusOnMouse();
		loadConfigurationPane();//init config pane
		loadAdvancedControlPane();
	...
	private void initializeListView()
	{
		...
		ObservableList<Label> labels = FXCollections.observableArrayList(
				new Label(ITEM_STRING_MULTIPLE_CHOICE),
				new Label(ITEM_STRING_CONFIGURATION),
				new Label(ITEM_STRING_ADVANCED_MODE));
		for (Label label : labels)
		{
			label.setFont(new Font(15));//change font of all labels to this...
		}
		listView.setItems(labels);//listView gets the content of labels
		listView.getSelectionModel().selectedItemProperty().addListener((name, oldSelect, newSelect) ->
		{
			// Change content of mainPane here.
			switch (newSelect.getText())//returns the text in the label that we passed off
			{
				case ITEM_STRING_MULTIPLE_CHOICE:
					if (!mainPane.getChildren().get(0).equals(multipleChoicePane))//if not currently on this page
						switchUsingTransition(multipleChoicePane);//go to this page with a transition (put the contents of this pane on the view pane)
					break;
				case ITEM_STRING_CONFIGURATION:
					...
					break;
				case ITEM_STRING_ADVANCED_MODE:
					if (!mainPane.getChildren().get(0).equals(advancedPane))
						switchUsingTransition(advancedPane);
					break;
			}
			drawer.close();//close side menu
		});
	}
	...
	{
		rawOutputDialog = new TextDialog("i>Skipper - Raw Output", this.getClass().getResource("/css/application.css").toExternalForm());
	...
	private void setSvgIcons()
	{
		...
		SVGGlyph startSvg = new SVGGlyph(
				"M 512 64 C 264.6 64 64 264.6 64 512 s 200.6 448 448 448 s 448 -200.6 448 -448 S 759.4 64 " + "512 64 Z m 0 820 c -205.4 0 -372 -166.6 -372 -372 s 166.6 -372 372 -372 s 372 166.6 372 "
						+ "372 s -166.6 372 -372 372 Z M 719.4 499.1 l -296.1 -215 A 15.9 15.9 0 0 0 398 297 v 430 c "
						+ "0 13.1 14.8 20.5 25.3 12.9 l 296.1 -215 a 15.9 15.9 0 0 0 0 -25.8 Z m -257.6 134 V 390.9 L " + "628.5 512 L 461.8 633.1 Z",
				Color.WHITE);
		startToggle.setText(null);
		setSVGForNode(startToggle, startSvg);

		// Stop Button
		SVGGlyph stopSvg = new SVGGlyph("M 512 64 C 264.6 64 64 264.6 64 512 s 200.6 448 448 448 s 448 -200.6 448 -448 S 759.4 64 512 64 Z m 0 "
				+ "820 c -205.4 0 -372 -166.6 -372 -372 c 0 -89 31.3 -170.8 83.5 -234.8 l 523.3 523.3 C 682.8 852.7 601 "
				+ "884 512 884 Z m 288.5 -137.2 L 277.2 223.5 C 341.2 171.3 423 140 512 140 c 205.4 0 372 166.6 372 372 " + "c 0 89 -31.3 170.8 -83.5 234.8 Z", Color.WHITE);
		stopButton.setText(null);
		setSVGForNode(stopButton, stopSvg);

		// Reset Button
		SVGGlyph resetSvg = new SVGGlyph("M 909.1 209.3 l -56.4 44.1 C 775.8 155.1 656.2 92 521.9 92 C 290 92 102.3 279.5 102 511.5 C 101.7 743.7 289.8 932 "
				...
				+ "20.4 27.8 31.4 l -60.2 47 a 8 8 0 0 0 3 14.1 l 175.6 43 c 5 1.2 9.9 -2.6 9.9 -7.7 l 0.8 -180.9 c -0.1 -6.6 -7.8 -10.3 " + "-13 -6.2 Z", Color.WHITE);
		resetButton.setText(null);
		setSVGForNode(resetButton, resetSvg);

		// Raw Toggle
		SVGGlyph rawSvg = new SVGGlyph("M 688 312 v -48 c 0 -4.4 -3.6 -8 -8 -8 H 296 c -4.4 0 -8 3.6 -8 8 v 48 c 0 4.4 3.6 8 8 8 h 384 c 4.4 0 8 -3.6 "
				...
				+ "-32 H 168 c -17.7 0 -32 14.3 -32 32 v 784 c 0 17.7 14.3 32 32 32 h 272 c 4.4 0 8 -3.6 8 -8 v -56 c 0 " + "-4.4 -3.6 -8 -8 -8 Z", Color.WHITE);
	...
	{
		FXMLLoader loader = new FXMLLoader();//set resource type
		loader.setLocation(this.getClass().getResource("/views/ConfigurationPane.fxml"));//load screentype file
		ConfigurationPaneController controller = new ConfigurationPaneController();//make a new class that uses the ConfigurationPaneController java file
		controller.setEmulator(emulator);
		controller.setApplication(application);
		controller.setPrimaryViewController(this);//pass off parent class
		loader.setController(controller);//pass off our controller to the screen file
		try
		{
			configurationPane = loader.load();//try to load the whole thing to the anchor pad in the parent screen
		} catch (IOException e)
		{
			e.printStackTrace();
		}
	...
	private void loadAdvancedControlPane()
	{
		FXMLLoader loader = new FXMLLoader();
		loader.setLocation(this.getClass().getResource("/views/AdvancedControlPane.fxml"));
		AdvancedPaneController controller = new AdvancedPaneController();
		controller.setEmulator(emulator);
		controller.setApplication(application);
	...
	private void switchUsingTransition(Pane to)
	{
		...
		moveInTransition.setOnFinished(e -> mainPane.getChildren().remove(0));//once done, delete old pane
		moveOutTransition.setOnFinished(e ->
		{
			addToMainPaneContent(to);
			moveInTransition.play();//begin transition keyframes
		});
		moveOutTransition.play();//ditto
	}
