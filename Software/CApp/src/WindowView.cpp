...
void WriteToTerminal(const char* NewText)
{
	...
	if (NewText)
	{
		...
		//free the memory of the topmost pointer
		if (TerminalLines[0] != NULL)
		{
			free(TerminalLines[0]);
			TerminalLines[0] = NULL;
		}
		...
		//put new values in the bottom of the array, remove old stuff as needed
		TerminalLines[TERMINAL_MAX_BUFFER - 1] = (char*)malloc(strlen(NewText) + 1);//allocate new memory space (no need to free, we do that at space 0)
		if(TerminalLines[TERMINAL_MAX_BUFFER - 1] != NULL)
			strcpy(TerminalLines[TERMINAL_MAX_BUFFER - 1], NewText);//copy new text to memory
...
void PutSerialToTerminal(void)
{
	if (serialIsOpen() && serialDataAvalible())
	{
		WriteToTerminal(serialGet().data());
	}
}
...
void ShowBasicClicker(void)
{
	/*
	
	text with clicker ID (changeable)
...
void ShowConnect(void)
{
	//layout:
	/*
	will refresh serial ports when the tab is clicked

	will have drop-down menu for selecting COM/tty port

	will have one button that changes connect/disconnect on connection status

	will have status text
	...
	*/
	...
	{
		gBuddy.serialList = serialList();//get vector list of ports

		//print to terminal
		for (int i = 0; i < gBuddy.serialList.size(); ++i)
		{
			WriteToTerminal(gBuddy.serialList[i].c_str());
		}
	...
	//port list tab
	{
		...
		//the one that is currently selected		
		if (gBuddy.serialList.size())
			combo_label = gBuddy.serialList[gBuddy.selectedPort].c_str();
		else
			combo_label = "N.A.";
		ImGui::PushItemWidth(120);
		if (ImGui::BeginCombo("Select Serial Port", combo_label))
		{
			//list all avalible tracks
			for (int n = 0; n < gBuddy.serialList.size(); n++)
			{

				const bool is_selected = (gBuddy.selectedPort == n);

				//if the entry has been clicked on
				if (ImGui::Selectable(gBuddy.serialList[n].c_str(), is_selected))
					gBuddy.selectedPort = n;//pass the corresponding number to our perameters
	...
	static bool tryConnect = false;//should we run the connect function?

	//attempt to connect with response
	const char* buttonLabel;
	//only call once every ms
	/*
	if (tryConnect)
		switch (clickerConnect())
		{
		default:
		case 0://success
			tryConnect = false;
			gBuddy.connected = true;
			WriteToTerminal("Connect Success");
		case 1://waiting
			break;
		case 2://timeout error
			...
			break;
		case 3://failed to open port
			tryConnect = false;
			WriteToTerminal("Port Open Error");
			break;
		}
*/
	if (ImGui::Button(gBuddy.connected? "Disconnect":"Connect"))
	{
		if (gBuddy.connected)
		{
			//if (serialDisconnect() == false)
			//	return;//do not go further, we don't even have a serial connection
			serialDisconnect();
			gBuddy.connected = false;

		}
		else if(gBuddy.serialList.size() > gBuddy.selectedPort)//prevent out-of-range access
		{
			tryConnect = true;//try to connect with ACK above


			gBuddy.connected = serialConnect(gBuddy.serialList[gBuddy.selectedPort], BaudRate9600);
		}
...
{
	static bool showSerialTerminal = false;//if true, append terminal window
	...
	if (ImGui::BeginMenuBar())
	{
		if (ImGui::BeginMenu("File"))
		{
			...
			{
				ImGui::Text("Little Buddy Control\nby Dr_Glaucous (2023)");
				ImGui::Separator();
				ImGui::Text("Version: ");
			...
			ImGui::Checkbox("Show Serial Terminal", &showSerialTerminal);
...
bool TopFunction(void)
{
	...
	{
	case 0://start
		window = InitializeGLFW("Little Buddy Control");
